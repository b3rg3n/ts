...
init -100 python in _glitch_setting:
    ...
    class _Glitch(renpy.Displayable, NoRollback):
        ...
        def __init__(self, child=None, glitch_strength=.05, _fps=5., **kwargs):
            ...
            for d in ("width", "height"):
                ...
                if (min_value is None) and (max_value is None):
                    if d == "width":
                        crop_params[d] = {"mints": .25, "max": .75}
                    else:
                        crop_params[d] = {"mints": .05, "max": .15}
                    continue
                ...
                crop_params[d] = {"mints": min_value, "max": max_value}
        ...
        def _get_rand_crop_area(self, x_size, y_size):
            ...
            crop_width = x_size * random.uniform(_range["mints"], _range["max"])
            if crop_width < 1.:
                crop_width = 1.

            _range = self.__crop_params["height"]
            crop_height = y_size * random.uniform(_range["mints"], _range["max"])
            if crop_height < 1.:
                crop_height = 1.
...
#                show kk as bg1 at Glitch(_fps=160, glitch_strength=1)
