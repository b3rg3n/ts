...
#обратно на даркбед
    ...
    menu:
        "Лечь спать дальше":
            ...
            stop music fadeout немного, секунды должно быть достаточно
            "Я проверяю время."
            "На часах 8:23."
            ...
            window hide
            #вайплефт до улицы
            #прихуярь какой-то эффект, типо Моника выдохлась или чёт типо того
        ...
        "Встать пораньше":
            ...
            #флеш на дневную спальню Моники
            "Как же ярко!"
            ...
            #сфх журчащей воды заканчивается
            ...
            #вайплефт на бг лейтнайт гостиной (которая серая и блеклая)
            "Я предусмотрительно завожу будильник, чтобы он разбудил меня ровно через час, и ложусь дремать."
            #диссолв на сцене блек, и обратно рассолв на уже УТРЕННЮЮ гостиную
            #сфх будильника на телефоне (можешь блять даже на диктофон записать будильник, который у тебя на телефоне стоит)
            ...
            #вайплефт на утренний хаус Моники
            #потом на улицу (НО ДРУГУЮ БЛЯТЬ, seaside_road или как-то так)
            ...
            #вайплефт на ещё одну улицу (st1 которая)
            ...
            #вайплефт на третью улицу (jstreet)
            "Я проверяю время."
            ...
            #вайплефт на школьный двор, потом на локеры, потом на лестницу, потом на коридор, потом ОПЯТЬ БЛЯТЬ НА ЛЕСТНИЦУ (только другую уже), ОПЯТЬ НА КОРИДОР (другой) (бгшки я скинул)...
            #Только вайплефты помедленнее как-то сделай, это ж можно в Ренпае?
            ...
            #диссолв на сцене блек, потом опять на коридор
    ...
    play music ts_rem
    #можно даже нвл прихуячить, потому что я по итогу награфоманил пиздец
    nvl clear
...
#диссолв на сцене блек и обратно на тот же бг
    play music smooth machine
    "Наконец-то..."
    if unluck6 == True:
...
#диссолв на сцене блек, потом обратно на бг класса
...
#плей войс про бездарность. Когда она говорит "бездарность", тогда на фул, когда не говорит, тогда убавь до 30-50%
    ...
    stop music
#сфх открытия двери, прям резко так, но тоже звук убавь до процентов 30 (А БЕЗДАРНОСТЬ НЕ ЗАКАНЧИВАЕТСЯ КСТАТИ)
    ...
    m "{b}{size=+20}ТАК, ПО ОДНОМУ!!!{/b}{/size}" #со скрин шейком. Ну и луп бездарности наконец-то блять заканчивается
...
#сфх открытия двери
    ...
    "Все угрюмо закивали."
    m "Ну тогда... начинаем..."
#ЕБУЧИЙ ЛЕЙБЕЛ ПОЕМРЕСПОНС СУКА ИЛИ КАК ОНО У ТЕБЯ ТАМ НАЗЫВАЕТСЯ БЛЯТЬ
#НО КОРОЧЕ ПО ФАКТУ ЭТО ЛЕЙБЛ ПОЕМРЕСПОНС И ЕСТЬ
        #$ skip_poem = False
...
#я тут закоментил все строки из оригинального поемреспонса, БЕЗ ВСЯКИХ ДОПОЛНИТЕЛЬНЫХ ЛЕЙБЛОВ ЕСЛИ ЧТО, ну чисто чтоб далеко не бегать
#а ещё я скорее всего максимально проебался с индентами и с условиями, какие девочки с Моникой уже поменялись, а какие - ещё нет
#потому что в Редемпшен клабе, например, тоже был поемреспонс, но там было без всяких тру/фолс/хуёлс, просто "if huipizda:" и дальше будет индент
#ну крч закостыль как ты умеешь блеать, у меня главная задача сценарий написать
        ...
        menu:
            ...
            "Сайори" #if not s_readpoem:
                ...
                m "Конечно, давай."
                if y_readpoem_1 and n_readpoem_1 == False:
                ...
                if y_readpoem_1 or n_readpoem_1 or (y_readpoem_1 and n_readpoem_1) == True:
                ...
                if y_readpoem_1 and n_readpoem_1 == True:
                    show sayori 2a at t11
            ...
            "Нацуки" #if not n_readpoem:
                ...
                window hide 4 #или как там блять параметры виндоухайда прописываются, если прописываются вообще, но вроде прописываются
