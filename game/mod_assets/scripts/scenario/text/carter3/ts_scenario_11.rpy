...
label ts_scenario_11:
    ...
    em 1br "Я, конечно, всегда знала, что ты не особо отличаешься умом и сообразительностью, но чтобы {i}настолько{/i}..."
    ...
    em 2bd "Ладно, давай умываться уже."
    ...
    "Папа же, которому не совсем свойственно подмечать эмоциональные тонкости, такую резкую смену настроения просто не мог не заметить."
    ...
    "Ну да и ладно. Как говорится, что написано топором, не вырубишь и топором."
    ...
    "Мне же всё равно, я слишком сосредоточена на том, чтобы переписать стих как можно скорее."
    play sound zvonok
    "Чёрт, не успела..."
    ...
    if persistent.cens_mode == True:
        ...
        "Так и не вспомнив одну строку из стихотворения, я устало подумала «да пошло оно всё к чёрту» и написала первое попавшееся предложение, которое мне только пришло в голову."
    "Ну да и пусть. Всё равно эта строка была слишком бессмысленной, поэтому вспомнить целую строку у меня просто нет сил."
    ...
    "Сколько, кстати, мне здесь ещё сидеть?.."
    ...
    "Нет, всё-таки Юри гораздо более внимательная, чем мне изначально казалось."
    ...
    "Юри уже как будто бы и не было."
    "Ну, если читает и она, то наконец-то смогу почитать и я."
    show monika 2k at f11
    em "И сколько процентов от прочитанного ты хотя бы запомнишь? Один? Да?"
    show monika 2j at t11
    m "Отстань."
    "Поскольку Юри уже целиком и полностью погрузилось в чтение, остального мира для неё как будто не существует."
    ...
    m "{size=+10}ТАНЬ!{/size}" with vpunch
    show monika 2l at f11
...
label poemresponsesuka_loop:
    ...
    menu:
        ...
        "Сайори" if not c11sayourichecknul:
            ...
            s 4w "Тучки наоборот стали только гуще!"
            ...
            y "О, ты учишься не по дням, а по часам, даже выучила новые словечки вроде «стихосложение»."
        ...
        "Нацуки" if not c11natsukichecknul:
            ...
            "То есть, да, стихотворение написано в стандартном, если можно так выразиться, стиле Нацуки, и оно в целом похоже на «Эми Любит Пауков», который она писала в прошлую пятницу, но..."
            ...
            "Я, конечно, со слов Аки, и не самая догадливая, но даже я представляю, что она имеет в виду Юри."
...
label poemend_normal:
    ...
    show natsuki 2u at t33
    m "Я понимаю, что сама сказала о том, что «в моём клубе каждый мог бы выражаться так, как его душе будет угодно», но не стоит опускаться до таких крайностей."
