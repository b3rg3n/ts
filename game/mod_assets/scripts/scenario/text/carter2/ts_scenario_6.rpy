label ts_scenario_6:
    ...
    em 2bd "Поэтому, если и хочешь использовать всякие модные местоимения, которые в последнее время стали отдельной темой для беседы, просто используй {i}она/её{/i}."
    ...
    if unluck3 == True:
        ...
        m "Что?"
        em "Да так, неважно..."
        "Но не об этом. Хлопья с молоком и кофе, да."

    "В детстве я очень любила молоко. Прям настоящее, только сдоенное с коровы, и чуть-чуть подержанное в холодильнике."
    ...
    "В конце концов, я же на самом деле варить ту же гречку или макароны, а это уже открывает окно для возможностей. Добавить те же сосиски, и вот тебе приемлемый ужин."
    ...
    "Затем он берёт из холодильника колбасу и сыр, и аккуратными ломтиками кладёт по кусочку на каждый ломтик хлеба."
    ...
    "Нет, кофе тоже можно поставить, но он всё равно ещё очень горячий, а в условиях не самого большого стола, есть риск, что мой кофе разольётся."
    ...
    "Нет, я, конечно, знаю, что папа работает где-то в отделе опеки нашего города, а мама... мама что-то там проектирует для каких-то космических кораблей."
    "Отсюда и тот факт, что она постоянно в командировках, в том числе и заграничных."

    stop music fadeout 4

    "Но, несмотря на то, что она по сути зарабатывает на порядок больше папы, он на жизнь тоже не особо жалуется."
    ...
    ts_ft 1g "Ну вот и хорошо, пора бы и проявить свои президентские качества и полномочия."
    ...
    em "Они скорее придут только потому, что вы все милые девочки, и одна из них к тому же довольно популярная."
    ...
    em "Просто зови меня Аки. Проще, чем [em_name], всего три буквы, два слога, и даже отдалённо похоже на твоё же имя."
    ...
    em "Я тебе ещё раз говорю, просто зови меня Аки. Произносить это не сильно-то и дольше того, как ты меня называешь сейчас."
    "«Ладно, {i}Аки.{/i} Теперь довольна?!»"
    em "Вот так-то лучше!"
    em "Всё, не буду тебе мешать - с химией и биологией ты и так не особо ладишь, а я не хочу, чтобы ты ещё больше ненавидела эти предметы."
    ...
    n 2k "Честно говоря, я эту мангу уже когда-то начинала читать, ещё аж лет в десять, но тогда она мне показалась слишком скучной."
    ...
    "Ну-у-у... или почти. Я так и не дочитала где-то полсотни последних страниц."
    ...
    n 1c "На данный момент вышло примерно десять томов примерно по триста страниц каждый."
    ...
    "Чего?!"
    "Чтобы я читала мангу? Да немыслимо!"
    ...
    "Поверить не могу, я... и читаю мангу! Это же просто немыслимо!"
    ...
    "Поскольку Сайори уже видела меня в субботу, и {i}насмеялась{/i} надо мной ещё тогда, скорее всего, высмеивать меня она больше не будет."
    ...
    m "И поскольку наш клуб, хоть и новый, но всё же тоже официально зарегистрированный, нам нужно тоже подготовить хоть что-то, чтобы не ударить в грязь лицом."
    ...
    y 1j "{size=-10}Ты хотела сказать, только что прочитали...{/size}"
    ...
    "Точно так же, как в Дискуссионном клубе я на равных дискутировала с обычными членами, в Литературном я на равных с остальными пишу стихи."
    "..."
    ...
    em "Ничего! Просто ты бездарность."
    ...
    "Да, девочки могут не понять его смысл. Да что там девочки, даже я сама с трудом могу его понять!"
    ...
    "Да и я тоже, чтобы не выбиваться из компании, слегка хихикнула."
    "Юри казалась совершенно непоколебимой."

    show sayori 4n at t32

    "И пока Сайори не начала тыкать Юри пальцем и бог знает чем ещё, я решаю продолжать."
    ...
    m "Вот и отлично."
    m "Что же, сегодня мы поделиться уже не успеем..."
    ...
    if unluck_ball >= 4:
        ...
        em "Ну, кто знает..."
        $ TS.Screens(hpunch)
        em "Может, ты в коме находишься, а ущипнуть себя - это своего рода проверка на то, спишь ты или нет."
    ...
    ts_ft "А впрочем, можешь сама лучше позвонить!"
    ...
    "Ещё спустя минут пять дымящаяся картошка уже насыпана на двух тарелках."
    ...
    "Но с другой стороны, вполне логично, что темы были довольно сложными - в соответствии с его высшим образованием, которое тоже не из самых лёгких."
    "Нет, я не говорю о том, что кому-то учиться легче, а кому-то сложнее. В конце концов, все профессии нужны, все профессии важны."
    "Но успехов в учёбе добивались лишь единицы. К счастью, папа был одним из тех «единиц»."
    ...
    show momika 1c at t33

    m "Привет, мам!"

    show momika 2k at f33
    ...
    ts_mt 2d "Я переночую один последний раз, и в полночь понедельника у меня будет самолёт."

    show momika 3k at f33

    ts_mt "К вечеру я уже буду дома, когда ты как раз вернёшься со своего фестиваля!"

    show momika 3j at t33
    ...
    show momika 2n at f33

    ts_mt "Что вы готовите на фестиваль, кстати?"

    show momika 2n at t33
