label ts_scenario_8:
    ...
    "В итоге мы оба над этим просто посмеялись."
    ...
    show hiroto 1y at t11
    "Впрочем, как нам обоим показалось, беседа уже закончена. С этими словами мы заканчиваем есть и убираем тарелки в посудомойку."
    ...
    "Хорошо хоть я зонтик взяла. Даже не могу себе представить, какой же дурой я бы была, если бы всё-таки не решилась взять зонтик."
    ...
    "Следующим этапом обязательно должно быть разогнать всех этих бездарных учителей гуманитарных наук, и на их место поставить действительно хороших."
    show monika 2d at aki_spawn
    ...
    "Надо же... В такие моменты кажется даже куда более ответственней меня."
    ...
    "Я точную цитату не помню, но почерк у неё действительно очень неразборчивый."
...
#вайплефт на corridor-rain, потом на club-rain (ШЕЙДЕРЫ АХАХАХ НАКАНЕЦТАААААА БЛЕАТЬ)
#эмбиент дождя и рейндропс не заканчиваются кста
...
s "Моника, ты же раньше в Дискуссионном клубе была..."
s 2e "Никогда не поверю, что президент Дискуссионного клуба никогда не выступал публично!"
...
"Да и дискутируем мы в основном между самими членами клуба. А это же не сильно отличается от обмена стихами в Литературном?"
...
#НУ ТЫ УЖЕ ЗНАЕШЬ КРЧ БЛЯТЬ, ТО ЖЕ САМОЕ, ЧТО И В ПРОШЛЫЙ РАЗ, НИЧЁ БЛЯТЬ СЛОЖНОГО
#КСТАТИ Я ОПЯТЬ ВСЕ ИНДЕНТЫ ПРОЕБАЛ, НУ ДА И ПОХУЙ, МНЕ ГЛАВНОЕ ТЕКСТ НАПИСАТЬ, А ВСЕМИ ИНДЕНТАМИ-ХУЕНТАМИ УЖЕ ТЫ БУДЕШЬ ЗАНИМАТЬСЯ
play music t5
...
label poemresponses2suka:
    ...
    menu:
        ...
        "Однако в последний момент отказываюсь от этого решения."
...
В своей голове."""
    ...
    "За те четыре дня Сайори очень сильно продвинулась в стихосложении."
    ...
    show sayori 5a at t11
    if y_readpoem2 and n_readpoem2 == True:
    ...
    if s_readpoem_2 or y_readpoem2 or (s_readpoem_2 and y_readpoem2) == False:
    ...
    show natsuki 1m at f11
    n "Моника, спасибо тебе, конечно, но... пожалуй, я откажусь."
    n 1t "Во-первых, папа не поймёт, откуда у меня взялись новые навороченные тетрадки..."
    n 1q "А во-выторых, одноклассники засмеют меня ещё больше, если увидят новые тетради. А если они узнают, откуда именно взялись эти тетради, то я и тебе репутацию подпорчу."
    ...
    show natsuki 1za at t11
    "Бинго."
    if s_readpoem_2 or y_readpoem2 or (s_readpoem_2 and y_readpoem2) == True:
    ...
    n "Да без вопросов!"
    n 1y "Покажу тебе, как писать стихи кратко и чтоб сразу к делу!"
...
И я собираюсь всем глаза раскрыть."""
    ...
    n "А ещё, прошу заметить, стих получился не таким громоздким, как у тебя!"
    ...
    show natsuki 1j at t11
    #прихуярь раздвоенный текстбокс, типо в одной половине думает Моника, а во второй половине говорит Аки
    "Эй, это моя реплика!"
    em "Эй, это моя реплика!"
    #нормальный текстбокс возвращается
    "«А ты не забыла, что ты - это часть меня, поэтому любая твоя реплика становится моей?»"
    em "..."
    m "Л-ладно..."
    show natsuki 1k at f11
    n "Ну, я пойду тогда?"
    if s_readpoem_2 and y_readpoem2 == True:
        show natsuki 1za at t11
        ...
        hide natsuki
        #ПОТОМУ ЧТО КОГДА Я В СЕДЬМОЙ ГЛАВЕ НЕ ДОПИСЫВАЛ ТХАЙДЫ, ТЫ ТОЖЕ ТХАЙДЫ НЕ ДОПИСАЛ, ПОЭТОМУ ВОТ БЛЯТЬ, Я САМ ДОПИСАЛ
        #ТАМ, ГДЕ ТХАЙД ТАК И НЕ ПРОПИСАН, ЗНАЧИТ, ТАК И НАДО БЛЯТЬ, ИМИТАЦИЯ РЕЗКОГО ИЗМЕНЕНИЯ СУКА
    else:
        show natsuki 1j at t11
    ...
    show yuri 2c at t11
    if s_readpoem_2 or n_readpoem2 == True:
    ...
    show yuri 1g at t11
    "Однако я совершенно забыла о том, что Юри не из тех, кто понимает импровизационные шутки."
...
Цвет сердца моего - янтарь, а синий изумруд - его."""
    ...
    "Но кто знает, может, после фестиваля она и третью часть этого стихотворения напишет?"
    ...
    "«А это ещё что за намёки? И почему именно следующая неделя, а не любой другой период?»"
    em "Да так... мысли вслух..."
    "«...»"
    show yuri 2b at f11
    y "Спасибо, Моника."
    y 1f "Ну, я пойду?"
    show yuri 1e at t11
    if s_readpoem_2 and n_readpoem2 == True:
    ...
    #вайплефт на club-rain2, имитация более позднего часа в клубе блять (мы же рассказываем всем, шо у нас поделка, продуманная до мелочей, вот и соответствовать надо)
    #наконец-то блять. В третьем акте будет фул линейный поемреспонс, Юра/Нацуки/Сайра, либо Нацуки/Юра/Сайра
    #в любом случае, Сайра будет последней, чтобы сцена со спором по поводу стиля написания стихов случилась, и чтобы логично было
play music t8
show yuri 1e at t31
show sayori 1b at t32
show natsuki 1za at t33
"Все девочки выжидающе смотрят на меня."
"Давай, Моника, ты сможешь. Это в конце концов твой же клуб."
m "И-и-и... итак, ребята!"
...
y "Вообще..."
#небольшой визуальный гэг. Я его с Абридгеда гопнул
show yuri 1l at f51
...
#блюр всего, прям невъебенно мощный, ну или какой-то другой фильтр
#ну чтобы остальные докичи не думали, что Моника реально тугодумина, и пять минут над ответом думает, а типо весь диалог происходит у неё в голове за пару секунд
...
em "Конечно, это хорошая идея. Любая идея будет лучше твоей, мисс «девочки, я слишком бестолочь, чтобы сделать что-то стоящее, поэтому придумайте задание вместо неё»."
"«Х-х-х...»"
#снеп бек ту реалити
show yuri 1e at t31
show sayori 2o at t32
...
m "Поэтому можешь нарисовать листовки и буклетики? Только чтобы они поярче были, покрасочнее."
...
#ВОТ СТИХ ЕСЛИ ЧТО. МОЖЕШЬ ПОКАЗЫВАТЬ, МОЖЕШЬ НЕ ПОКАЗЫВАТЬ, ГЛАВНОЕ, ЧТОБЫ КОНТЕКСТ БЫЛ ПОНЯТЕН, О КАКОМ СТИХЕ ИДЁТ РЕЧЬ
    poem_m2 = Poem(
    author = "monika",
    title = "Спаси меня",
    text = """\
...
show sayori 3i at t32
    ...
    #ЭТО СТИХ ИЗ РЕДЕМПШЕН КЛАБА, НО ДАЖЕ В ОРИГИНАЛЕ ЕСЛИ ИДТИ ПО РУТУ САЙРЫ, ТО ОНА В КАКОЙ-ТО МОМЕНТ САМА ЭТОТ СТИХ ВЫПЛЮНЕТ
    poem_su = Poem(
    author = "sayori",
    title = "Мой Луг",
    text = """\
...
show yuri 2s at t31
"Я не обратила внимания на ремарку Юри, хотя и примерно поняла намёк: ножи, резать, соблазн, возбуждение - намёк прямее некуда."
...
n 2d "Что же, не буду заморачиваться и зачту стих, который я написала на сегодня."
    ...
    #Я УЖЕ ПОКАЗЫВАЛ СТИХ В ЭТОЙ ГЛАВЕ, ДА И ПИХУЙ
    poem_n2 = Poem(
    author = "natsuki",
    title = "Эми любит пауков",
    text = """\
...
show yuri 1g at t31
m "{size=+10}{b}ДА ПОГОДИТЕ ВЫ ВСЕ!{/b}{/size}"
stop music
show yuri 3n at h31
...
m "Поэтому в субботу я сначала пойду к Сайори, а затем к Юри, а в воскресенье буду помогать Нацуки, ну чтобы кексы к этому моменту не превратились в сухари."
...
$ m_name = "Моника"
m "У нас впереди ещё очень важные выходные, и за эти выходные мы должны показать, что у нас клуб, достойны того, чтобы в него вступить."
