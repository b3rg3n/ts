...
#микрогличик добавь, прям ну совсем милипиздрический
...
#вайплефт на экстерьер дома Сайры/гг | присрать анимацию
    ...
    if chess_tutor == True:
        ...
        cm 2n "В-в общем, ладно, наслаждайся игрой."
        cm 3b "Кстати, эта игра списана с игры реального человека, сценариста этого мода!"
    ...
    if chess_tutor == True:
        ...
        cm 2d "Закрытые же дебюты начинаются с ходов 1. d4 d5."
        ...
        cm 3k "А поскольку в моде не было и не будет ни одной игры в закрытых дебютах, мы тоже будем считать, что все дебюты, которые начинаются не с хода 1. е4 - закрытые."
    ...
    if chess_tutor == True:
        ...
        cm 3d "Что нужно сделать для того, чтобы они вступали в игру чуть раньше?"
        cm 3b "Правильно, рокировка."
        cm 2d "Рокировка - это специальный ход короля и одной из ладей, который, по сути, ломает все правила шахмат."
        cm 3d "Тут тебе и король, который ходит сразу на две клетки вместо одной, и в принципе игрок ходит сразу двумя фигурами..."
        cm "И по итогу король ходит на две клетки, а ладья становится сразу за королём, образуя своеобразный замок из пешек и ладьи."
        cm 3i "Но у такой хитрости есть ряд ограничений."
        cm 2d "Во-первых, до рокировки нельзя делать ходов ни королём, ни ладьёй, с которой король хочет рокироваться."
        cm 4d "Даже если какая-то из фигур походит, а затем вернётся на своё изначальное место, право на рокировку теряется."
        cm 2d "Во-вторых, рокироваться нельзя, если королю объявлен шах. Рокироваться, чтобы уйти от шаха, тоже нельзя."
        cm "И наконец, при рокировке король не должен быть под шахом ни в одной из клеток, применяемых при рокировке."
        cm 2n "Ну и ещё в турнирных играх и при игре на онлайн-ресурсах король должен ходить первым. Если первой походит ладья, то это просто будет считаться обычным ходом ладьи, а право на рокировку потеряется..."
        show momika at thide
        hide momika
    ...
    if chess_tutor == True:
        ...
        cm 2d "Кстати, существует два типа рокировок: короткая и длинная. Короткая делается на королевском фланге, а длинная - на ферзевом."
        ...
        cm 4d "Однако в некоторых дебютах предпочтительнее всё же длинная рокировка, потому что задействованы все фигуры с ферзевого фланга."
        cm 2d "А если оба игрока рокируются в разные стороны, то это обычно означает драку не на жизнь, а на смерть. "
        extend 3l "В шахматах, конечно же. В жизни шахматисты очень миролюбивые."
        cm 3b "Ладно, не буду тебе мешать, продолжай смотреть игру!"
        show momika at thide
        hide momika
    "..."
    show layer screens at ts_hidescreens
    ...
    "Это и не удивительно... Я уже так удивилась за прошедшие две недели, что внезапное умение Аки играть в шахматы - это наименьшее из всех зол."
    show layer screens at ts_hidescreens
    " {w=1.0}{nw}"
    ...
    n "Кексы! Пока что никто не жаловался, и не думаю, что кто-то пожалуется, ведь будет работать профессионал!"
    ...
    m "Ну, а нам и не нужно очередное произведение искусства. Просто нарисуй несколько листовок и буклетиков, распечатай пару сотен штук, и на этом всё. Думаешь, справишься?"
    ...
    em "Ну, обижайся и дальше, факта это не изменит."
    "«Х-х-х...»"
    ...
    "Однако эта кудря только улыбнулась, как будто увидев там что-то очень приятное."
    ...
    em "Одно убийство изощрённее другого."
    show monika 2h at t11
    m "Ну а что мне ещё делать, если я уже всё сделала всё мыслимое и немыслимое?"
    show monika 2i at f11
    em "А ты не думала, что, вместо того, чтобы убивать и насиловать милых девочек, ты могла бы заняться чем-нибудь полезным?"
    ...
    show kuninobu 3e4 at f11
    k "Нет, конечно, похоронили меня не в школьной форме, и со всеми почестями, как полагается..."
    k 2d "Но для простоты и удобства я {i}хожу{/i} именно в ней."
