label ts_scenario_1:
    ...
    if persistent.cens_mode == True:
        ...
        "Я и так, бл*ть, знаю, что время идёт, не надо мне лишний раз об этом напоминать."
    ...
    if persistent.cens_mode == True:
        ...
        "Но неужели нет ни единой живой души во всей этой бл*дской школе, которой бы была интересна литература?"
    ...
    "Не то, чтобы мне это было совсем непривычно, но после шума и гомона Дискуссионного клуба Литературный клуб кажется каким-то особенно пустым."
    ...
    "(правда, я честно не знаю, как это отличается от тех же листовок, если вообще отличается, ну да и пусть."
    "Эту идею всё равно предложила Сайори, она ещё не знает, через какие тернии мне пришлось пройти),"
    "и многое другое."
    ...
    m "Нет, если, конечно, ты приготовишь, то без проблем, но я тебе по секрету скажу, что я довольно паршиво готовлю."
    ...
    s "Потому что я готовлю даже хуже, чем ты. Вне зависимости от того, как плохо готовишь ты, я уверяю, что я готовлю ещё хуже."
    ...
    "Вместо ответа Сайори начинает слезиться."
    ...
    s "А затем я подумала, что раз уж я такая бесполезная, если мне в любом случае будет плохо, то нужно, чтобы хотя бы другим было хорошо."
    ...
    show sayori 3p at t11

    m "Ну уж нет!" with vpunch
    ...
    "Сайори впервые за сегодняшнюю встречу улыбается мне."
    
    show sayori 2zc at t11
    ...
    "Что же, надеюсь, что эти «серые тучки» наконец рассеются, и тучи наконец принесут нам обеим радугу…"
    ...
    "Но это всё равно можно сказать довольно крепкой дружбой."
    ...
    s "Как что? Пока все обедали, она читала книжку!"
    ...
    show sayori 5 at t11

    extend " Не знаю..." with vpunch
    ...
    m "А с учётом того, что сегодня пятница, то и вообще мы увидим её не раньше понедельника."

    "Сайори хмуро смотрит на меня."
    "Да она сама виновата!" with vpunch
    ...
    show hiroto 1a zorder 2 at t11
    ...
    "Но в итоге мы тоже все с этого посмеялись."
    ...
    window hide
    show zatemnenie
    show screen chp_text_24
    pause
    hide zatemnenie
    hide screen chp_text_24
    ...
    "Как для мужчины, который попросту не знает обо {b}всех{/b} тонкостях, таких как матка и менструация, например, он был очень хорош."
    ...
    "Да и о её внешности мы же совершенно не знаем."
