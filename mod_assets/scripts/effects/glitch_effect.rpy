...
init -666 python in _glitch_setting:
    ...
    class _Br_Glitch(renpy.Displayable, NoRollback):
        
        def __init__(self, child=None, glitch_strength=.05, _fps=5., **kwargs):
            
            super(_Br_Glitch, self).__init__()
            self._m1_br_script__child = None
            if child is not None:
                child = renpy.easy.displayable(child)
                if not isinstance(child, renpy.display.core.Displayable):
                    raise TypeError(__("Передан не 'displayable' объект."))
                self._m1_br_script__child = child
            
            self._m1_br_script__init_kwargs = {
                "glitch_strength": glitch_strength,
                "_fps": _fps
            }
            self._m1_br_script__init_kwargs.update(kwargs)
            ...
            self._m1_br_script__crop_params = crop_params
            ...
            self._m1_br_script__color_range = (
                renpy.color.Color((color_range1 or "0000")),
                renpy.color.Color((color_range2 or "#fff8")),
            )
            
            self._m1_br_script__fps = float(_fps)
            if self._m1_br_script__fps <= .0:
                raise ValueError(__("Некорректное значение '_fps'."))
        
        @property
        def glitch_strength(self):
            return self._m1_br_script__glitch_strength
        
        @glitch_strength.setter
        def glitch_strength(self, new_value):
            self._m1_br_script__glitch_strength = float(new_value)
            if not (.0 <= self._m1_br_script__glitch_strength <= 1.):
                raise ValueError(__("Некорректное значение 'glitch_strength'"))
        
        def __call__(self, child):
            
            kwargs = self._m1_br_script__init_kwargs.copy()
            return _Br_Glitch(child, **kwargs)
        
        def event(self, ev, x, y, st):
            if self._m1_br_script__child:
                return self._m1_br_script__child.event(ev, x, y, st)
        
        def visit(self):
            if self._m1_br_script__child:
                return [self._m1_br_script__child]
            return []
        
        def _get_color(self):
            return renpy.color.Color(
                tuple(
                    map(lambda x: random.uniform(*x), zip(*self._m1_br_script__color_range))
                )
            )
        ...
        def _get_rand_crop_area(self, x_size, y_size):
            ...
            _range = self._m1_br_script__crop_params["width"]
            crop_width = x_size * random.uniform(_range["min"], _range["max"])
            if crop_width < 1.:
                crop_width = 1.
            
            _range = self._m1_br_script__crop_params["height"]
        ...
        def render(self, *render_args):
            if not self._m1_br_script__child:
                return renpy.Render(1, 1)
            child_render = renpy.render(self._m1_br_script__child, *render_args)
            _num = random.randint(15, 50)
            result_render = self.divide(child_render, _num, *render_args)
            _redraw_time = (1. / self._m1_br_script__fps) * random.uniform(.75, 1.25)
            renpy.redraw(self, _redraw_time)
            return result_render

    setattr(store, "br_glitches", _Br_Glitch)
...
#                show kk as bg1 at br_glitches(_fps=160, glitch_strength=1)
#                $ renpy.pause(1.1, hard=True)
#                play music crysis3epilogue fadein 10
#                scene kk
#                show dust1
#                show dust2
#                show dust3
